...
--[=[
	...
	A library of table manipulation functions.
	Forked from Sleitnik's TableUtil
]=]
...
function TableUtil.ShallowReconcile<S, T>(src: S, template: T): S & T
	assert(type(src) == "table", "First argument must be a table")
	assert(type(template) == "table", "Second argument must be a table")
	...
	for k, v in template do
		...
		if sv == nil then
			...
			--elseif type(sv) == "table" then
			--	if type(v) == "table" then
			--		tbl[k] = TableUtil.ShallowReconcile(sv, v)
			--    else
			--        warn("Mismatched Types: ", k, " in template is a table, but in source is not")
			--	end
		end
	end
...
--[=[
	Converts an array into a dictionary using a given function to generate a new Key and Value.
	@param array {T} -- The array to parse
	@param fn (T, number) -> (K, V) -- A function to transform the index
	@return {[K]: V} -- The new dictionary
]=]
function TableUtil.ToDict<T, K, V>(array: { T }, fn: (T, number) -> (K, V)): { [K]: V }
	local dict = {}
	for index, value in ipairs(array) do
		local k, v = fn(value, index)
		dict[k] = v
	end
	return dict
end
...
--[=[
	A simple wrapper for making a weak table.
	By default just the keys are made weak, but the second argument allows specifying the mode.
	@param tbl T? -- The table to make weak
	@param mode string? -- The mode to use for the weak table
	@return T & tbl -- The weak table
]=]
function TableUtil.Weak<T>(tbl: T?, mode: string?): T & tbl
	assert(not mode or mode == "k" or mode == "v" or mode == "kv", "Invalid mode")
	assert(not tbl or (typeof(tbl) == "table" and not getmetatable(tbl)), "Invalid table")
	return setmetatable(tbl or {}, { __mode = mode or "k" })
end
